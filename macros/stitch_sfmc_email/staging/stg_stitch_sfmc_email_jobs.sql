...
select distinct
    ...
    safe_cast(
        {{ dbt_date.convert_timezone("cast(coalesce(sched_dt, pickup_dt) as timestamp)") }} as timestamp
    ) as best_guess_timestamp,
    safe_cast( {{ dbt_date.convert_timezone("cast(coalesce(sched_dt, NULL) as timestamp)") }} as timestamp) as scheduled_timestamp,
    safe_cast({{ dbt_date.convert_timezone("cast(pickup_dt, NULL as timestamp)") }} as timestamp) as pickup_timestamp,
    safe_cast({{ dbt_date.convert_timezone("cast(delivered_dt, NULL as timestamp)") }} as timestamp) as delivered_timestamp,
