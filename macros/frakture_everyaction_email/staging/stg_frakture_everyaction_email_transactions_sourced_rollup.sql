...
SELECT SAFE_CAST(message_id AS STRING) AS message_id,
    ...
    SAFE_CAST(NULL AS int) AS total_donors,  -- doesn't seem available in Frakture email_summary_pivot tables
    ...
    SAFE_CAST(NULL  AS numeric) AS new_monthly_revenue, -- doesn't seem available in Frakture email_summary_pivot tables
    SAFE_CAST(NULL  AS int) AS new_monthly_gifts, -- doesn't seem available in Frakture email_summary_pivot tables
