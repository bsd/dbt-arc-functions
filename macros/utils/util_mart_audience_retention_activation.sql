...
, join_cohorts as (
    select 
        ...
        '{{ ret_or_act }}' || LPAD(CAST(coalesce(rev_by_cohort.month_diff_int, month_diff_sequence.month_diff_int) AS STRING), 2, '0') as {{activation_or_retention}}_str,
        coalesce(rev_by_cohort.month_diff_int, month_diff_sequence.month_diff_int) as month_diff_int,
