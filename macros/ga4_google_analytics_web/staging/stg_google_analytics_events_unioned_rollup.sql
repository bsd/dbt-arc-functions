{% macro create_stg_google_analytics_events_unioned_rollup(
    ...
    select
        ...
        count(
            distinct case
                when event_name = 'view_search_results' then user_id else null
            end
        ) as unique_searches,
        ...
        sum(
            case when event_name = 'sub_menu_expand' then 1 else 0 end
        ) as sub_menu_expands,
        count(
            distinct case when event_name = 'sub_menu_expand' then user_id else null end
        ) as unique_sub_menu_expands,
        sum(
            case when event_name = 'user_engagement' then 1 else 0 end
        ) as user_engagements,
        count(
