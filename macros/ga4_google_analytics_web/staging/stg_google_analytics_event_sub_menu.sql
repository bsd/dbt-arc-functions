{% macro create_stg_google_analytics_event_sub_menu(
...
SAFE_CAST(nav_value.value.string_value as STRING) as top_nav_name,
SAFE_CAST(sub_value.value.string_value as STRING) as sub_menu_name,
...
--cross join unnest(event_params) refer_value
 ...
 cross join unnest(event_params) nav_value
 cross join unnest(event_params) sub_value
...
--and click_value.key = 'link_domain'
    and nav_value.key = 'top_nav_name'
    and sub_value.key = 'sub_menu_name'
