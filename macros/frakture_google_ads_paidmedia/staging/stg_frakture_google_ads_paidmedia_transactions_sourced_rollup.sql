{% macro create_stg_frakture_google_ads_paidmedia_transactions_sourced_rollup(
...
SELECT SAFE_CAST(ad_summary.message_id AS STRING) AS message_id,    SAFE_CAST(ad_summary.date AS TIMESTAMP) AS date_timestamp,
    ...
    SAFE_CAST(NULL AS int) AS total_donors,  -- doesn't seem available in Frakture ad_summary tables
