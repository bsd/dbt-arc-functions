...
def get_create_or_update():
    ...
    while create not in ("c", "u"):
        print(
            "Enter c to (c)reate new standard models or u to (u)pdate standard models."
        )
        create = input()
    return create == "c"
...
def get_destination():
    ...
    while (
        ...
        and not destination_path.endswith(path.join("dbt-arc-functions", "models"))
    ):
        print(
...
def get_list_of_sources(macros_path):
    ...
    list_of_sources = [
        directory
        for directory in os.listdir(macros_path)
        if path.isdir(path.join(macros_path, directory))
    ]
...
def overwrite_choice(source_file_path, output, destination_file_path):
    ...
    with open(destination_file_path, "r", encoding="utf-8") as d:
        d_text = list(
            filter(lambda x: not x.startswith("-- depends_on:"), d.readlines())
        )
        differences = list(
            difflib.unified_diff(
                d_text,
                output,
                fromfile=destination_file_path,
                tofile=source_file_path,
                lineterm="\n",
            )
        )
        if not differences:
    ...
    while overwrite not in ("o", "k"):
        print(
            "Enter o to (o)verwrite your model with the standard model or k to (k)eep your model."
        )
        overwrite = input()
    return overwrite == "o"
...
def create_or_update_docs(docs_path, destination_path):
    ...
    if docs_model != schema_dict["models"][existing_model_index]:
        ...
        schema_dict_model_set = set(schema_dict["models"][existing_model_index])
        print(
...
def delete_non_standard_model_choice(destination_file_path):
    ...
    print(
        "File contents printed above. This file does not exist in the standard models."
    )
    ...
    for _, _, files in os.walk(source_path):
        for file in files:
            if file.endswith(".sql") or file == f"{source}.yml":
                ...
                write_to_file(
                    source_file_path, destination_path, file, source, model_type, create
                )
                if file.endswith("sql"):
...
def loop_through_list_of_models(
    ...
    for model_type in list_of_models:
        ...
        if (
            ...
            and create
            and not destination_path.endswith("sources")
        ):
            print(
...
def main(dbt_models_path=""):
    ...
    try:
        ...
        process_sources(
            sources_wanted, list_of_sources, macros_path, create, dbt_models_path
        )
