...
def get_access_token():
    print("\nPlease go to this website and copy your access token:")
    print("https://github.atom.io/auth/github_package/token")
    print("\nPaste your access token here: ")
    a_t = input()
    if a_t:
        return a_t
    else:
        print('You did not provide an access token.')
        print()
        return get_access_token()


def clone_repo(a_t):
    git_url = f'https://{a_t}:x-oauth-basic@github.com/bsd/arc-dbt-functions'
    Repo.clone_from(git_url, 'tmp-arc-dbt-functions')
...
def process_sources(sources_wanted, list_of_sources, dbt_string,macros_path,create):
    model_types = ['source', 'staging', 'marts']
    rx = re.compile(r"\{% macro (.*?) %\}", re.DOTALL)
    git_prepend = 'https://github.com/bsd/arc-dbt-functions/blob/main/macros'
    ...
    for source in sources_wanted:
        ...
        for model_type in os.listdir(path.join(macros_path, source)):
            ...
            destination_path = path.join('models', model_type, source)
...
if __name__ == '__main__':
    macros_path = path.join('tmp-arc-dbt-functions', 'macros')
    create = get_create_or_update()
    try:
        a_t = get_access_token()
        clone_repo(a_t)
        list_of_sources = get_list_of_sources(macros_path)
        sources_wanted = get_sources_wanted(list_of_sources)
        process_sources(sources_wanted,list_of_sources,dbt_string,macros_path,create)
    except Exception as e:
        print(e)
        pass
    rmtree('tmp-arc-dbt-functions')
