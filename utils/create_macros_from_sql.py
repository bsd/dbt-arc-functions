                ...
                if not text.strip().startswith("{% macro"):
                    ...
                    if len(references) > 0:
                        ...
                        if len(references) == 1:
                            ...
                            text = ref_pattern.sub("{{ ref(reference_name) }}", text, 1)
                    ...
                    if len(sources) > 0:
                        ...
                        # Handle the case where there are multiple sources
                            ...
                            for i, (source_name, source_table_name) in enumerate(
                                sources
                            ):
                                ...
                                # Add the source argument and its value to the
                                # output
                                output += f"\n    {source_argument_name}={source_name},"
                                # Add the source table argument and its value
                                # to the output
                                output += f"\n    {source_table_argument_name}={source_table_name},"
                                # Replace the source in the text with the macro
                                # call
                                text = source_pattern.sub(
                                    f"{{{{ source({source_argument_name},{source_table_argument_name}) }}}}",
                                    text,
                                    1,
                                )
                    ...
                    output += "\n{% endmacro %}"
            with open(
                os.path.join(directory, "macros", file_name), "w", encoding="utf-8"
            ) as f:
